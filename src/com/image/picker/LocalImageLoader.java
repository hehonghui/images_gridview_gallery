...
public class LocalImageLoader {
     ...
     * 图片请求列表，用于调度
     */
    private ArrayList<ImageLoadRequest> mImagesList = new ArrayList<ImageLoadRequest>();

    /**
     * 处于正在请求状态的请求列表
     */
    private ArrayList<ImageLoadRequest> mOnLoadingList = new ArrayList<ImageLoadRequest>();
     ...
     * @return
     */
    public Bitmap displayImage(final String path, final ImageCallBack mCallBack) {
        return this.displatImage(path, null, mCallBack);
    }
    ...
    public Bitmap displatImage(final String path, final Point point, final ImageCallBack callBack) {
        // 先获取内存中的Bitmap
        Bitmap bitmap = mImageCache.get(path);

        // 若该Bitmap不在内存缓存中，则将其加入到调度任务列表中
        if (bitmap == null) {
            addImageRequest(new ImageLoadRequest(path, point, callBack));
        }
        return bitmap;
    }
     ...
     * 添加图片请求任务
     */
    private void addImageRequest(ImageLoadRequest item) {
     ...
     * @param path
     */
    private void removeImageRequest(ImageLoadRequest item) {
    ...
    private void dispatch() {
        ...
        if (mImagesList.size() < spareThreads) {
            for (ImageLoadRequest item : mImagesList) {
                execute(item);
            }
     ...
     * @param request
     */
    private void execute(final ImageLoadRequest request) {
        // 如果该图片正在请求，忽略之。
        ...
        mThreadPool.execute(new Runnable() {
            ...
            public void run() {
                ...
                // 先获取图片的缩略图
                Bitmap mBitmap = decodeBitmapFromFile(request.getPath(), size.x, size.y, false);
                ...
                // 将图片加入到内存缓存
                mImageCache.put(request.getPath(), mBitmap);
            }
        });
    }
    ...
    private Bitmap decodeBitmapFromFile(String path, int viewWidth, int viewHeight,
        ...
        // 设置缩放比例
        options.inSampleSize = computeInSmallSize(options, viewWidth, viewHeight);
        if (!isHighQuality) {
    ...
    private static class ImageHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (request != null) {
                ...
                request.getCallBack().onImageLoaded((Bitmap) msg.obj, request.getPath());
            }
        }
    }
